...
const ProductCarousel = ({ items = [], renderItem, slidesToShow = 6 }) => {
  // Changed slidesToShow to 6
  ...
  return (
    <div className="relative px-4 sm:px-8 md:px-12 lg:pl-20 lg:pr-[6rem]">
      ...
      <div className="flex gap-2 transition-transform duration-300 ease-in-out">
        {getVisibleItems().map((item, index) => (
          <div
            key={`${item.id || index}`}
            className="flex-shrink-0 w-full sm:w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/6" // Adjusted to w-1/6 for 6 items
          >
            {renderItem(item)}
          </div>
        ))}
      ...
      {validItems.length > responsiveSlidesToShow && (
        <button
          onClick={next}
          className="absolute right-0 sm:right-2 md:right- top-1/2 transform -translate-y-1/2 z-20 cursor-pointer focus:outline-none"
          aria-label="Next"
        >
          <Image
            src={Butterflies5}
...
const PopularProducts = ({ token = "irrv211vui9kuwn11efsb4xd4zdkuq" }) => {
  ...
  const renderProduct = (product) => {
    ...
    return (
      <div className="w-full relative h-full pt-6 sm:pt-8 md:pt-12">
        ...
        <div className="bg-[#f9e2b2] rounded-t-3xl rounded-b-[45%] h-40 sm:h-48 md:h-64 flex items-center justify-center mx-auto w-[54%] xs:w-[90%] sm:w-full">
          <div className="relative w-full h-full mt-4 sm:mt-6 md:mt-8">
            <Image
              ...
              layout="fill"
              objectFit="contain"
              className="scale-110 transition-transform duration-300 hover:-translate-y-14 hover:scale-115 max-w-[75%] max-h-[83%] mx-auto"
              onError={(e) => {
        ...
        <p className="text-[#db5c3c] font-gotham-medium text-sm sm:text-base md:text-pt-16 mt-2 sm:mt-4 text-center px-2 sm:px-4 py-1 whitespace-nowrap overflow-hidden text-ellipsis">
          {product.name}
        </p>
        <button
          onClick={() => handleCustomizeClick(product)}
          className="bg-[#197d8e] text-sm sm:text-base md:text-pt-16 font-gotham-rounded-bold mt-2 sm:mt-4 mx-auto block text-white px-3 sm:px-4 py-1 rounded-full whitespace-nowrap mb-2 sm:mb-4"
        >
          Customise
        </button>
      </div>
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 md:px-8 lg:px-">
      <h3 className="font-gotham-rounded-bold text-lg sm:text-2xl md:text-pt-20 text-center text-[#db5c3c] md:text-[#db5c3c] mt-4 sm:mt-6 md:mt-8 relative z-10">
      ...
      <div className="relative z-10 max-w-8xl mx-auto">
        {products.length > 0 && (
          <ProductCarousel
            ...
            slidesToShow={6} // Updated to display 6 cards
            variant="products"
          />
        )}
      </div>
