...
const BlogCard = ({ title, imageSrc, readMoreLink, index }) => {
  ...
  return (
    <Link href={`/blog/${slug}`} passHref>
      <div
        className={`relative mt-20 mb-20 cursor-pointer ${backgroundColor} shadow-lg p-4 pt-24 pb-6 h-[20rem] w-[20rem] flex flex-col justify-between m-4`}
      >
        ...
        <div className="flex flex-col flex-grow justify-between">
          ...
          <div className="text-center">
            <a
              href={readMoreLink}
              className="inline-block text-3xl text-black px-4 py-2 rounded-full font-semibold text-xl hover:text-white transition duration-300"
            >
              Read More
            </a>
          </div>
