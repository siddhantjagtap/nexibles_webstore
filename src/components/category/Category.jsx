...
export default function CelebrationCategoryPage() {
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-white pt-16 relative my-4">
      {/* Mobile menu button */}
      <button
        className="md:hidden fixed top-20 right-4 z-20 p-2 rounded-full shadow-lg bg-[#197d8e]"
      ...
      {/* Sidebar */}
      <aside
        className={`w-full md:w-[15%] bg-[#d88473] p-4 md:p-6 fixed md:static top-0 left-0 h-full md:h-auto overflow-y-auto transition-transform duration-300 ease-in-out z-10 ${
          isMobileMenuOpen ? "translate-x-0" : "-translate-x-full"
        } md:translate-x-0 rounded-lg md:rounded-[2.5rem]`}
      >
        {categoryData.map((category, index) => (
          ...
          >
            <div className="rounded-full bg-white w-12 h-12 md:w-[120%] p-2 flex justify-center items-center overflow-hidden">
              <img
      ...
      {selectedCategory && (
        <main className="w-full md:w-[80%] p-4 md:mt-0">
          <CategoryBanner
            className="w-full bg-[#f9e2b2] p-4"
            categoryName={selectedCategory.name}
          />

          {/* Product display grid */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 mt-8 md:mt-16 px-4">
            {products.length > 0 ? (
              products.map((product) => (
                <div key={product.id} className="relative h-full">
                  <div className="bg-[#f9e2b2] rounded-t-3xl rounded-b-3xl h-64 w-full flex items-center justify-center">
                    <div className="relative w-full h-full p-2">
                      <Image
                        ...
                        layout="fill"
                        objectFit="contain"
                        className="transition-transform duration-300 hover:scale-110 max-w-[90%] max-h-[90%] mx-auto"
                      />
                    </div>
                  </div>
                  <p className="text-[#db5c3c] mt-4 text-center font-bold text-lg">
                    {product.name}
                  </p>
                  <Link
                  ...
                  >
                    <button
                      className="bg-[#197d8e] mt-4 text-white px-6 py-2 rounded-full font-bold text-base mx-auto block"
